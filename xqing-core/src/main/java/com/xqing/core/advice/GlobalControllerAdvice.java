package com.xqing.core.advice;

import com.xqing.core.enums.ResultCode;
...
public class GlobalControllerAdvice {
	...
	public ResponseEntity<Response<?>> badRequestHandler(Exception e) {
		log.warn(e.getMessage());
		if (e instanceof HttpMessageNotReadableException || e instanceof HttpRequestMethodNotSupportedException
				|| e instanceof MethodArgumentTypeMismatchException) {
			return ResponseEntity.badRequest().body(Response.fail(BasicErrCode.INVALID_PARAMETER));
		}

		if (e instanceof MissingServletRequestParameterException) {
			return ResponseEntity.badRequest()
					.body(Response.fail(BasicErrCode.INVALID_PARAMETER.getCode(), "Required parameter '"
		...
		if (e instanceof ConstraintViolationException) {
			...
			return ResponseEntity.badRequest().body(Response.fail(BasicErrCode.INVALID_PARAMETER.getCode(), message));
		}

		if (e instanceof BizException) {
			BizException exception = (BizException) e;
			return ResponseEntity.ok().body(Response.fail(exception.getErrCode(), exception.getErrMessage()));
		}
		...
		if (e instanceof ExcelException) {
			...
			return ResponseEntity.ok()
					.body(Response.fail(BasicErrCode.BIZ_ERROR.getCode(), exception.getMessage()));
		}
		...
		return ResponseEntity.badRequest().body(Response.fail(BasicErrCode.INVALID_PARAMETER.getCode(),
				fieldError.getField() + " " + fieldError.getDefaultMessage()));
	}

	@ExceptionHandler({ Throwable.class })
	public ResponseEntity<Response<?>> serverErrorHandler(Throwable e) {
		log.error(e.getMessage(), e);
		return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
				.body(Response.fail(ResultCode.INTERNAL_SERVER_ERROR));
	}
}
